...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white selection:bg-blue-100">
      ...
      {/* 1. FLOATING SUPPORT ICON (New Feature) */}
      <motion.div 
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
        className="fixed bottom-10 right-10 z-[100] cursor-pointer group"
      >
        <div className="absolute -top-12 right-0 bg-slate-900 text-white text-[10px] font-black px-4 py-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap uppercase tracking-widest">
          Chat with Support
        </div>
        <div className="bg-blue-600 w-20 h-20 rounded-full flex items-center justify-center shadow-2xl shadow-blue-400 animate-pulse">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-xl border-b border-slate-100 px-10 py-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-12 items-center">
          ...
          <Link href="/list-your-pg" className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-xs shadow-lg uppercase">List Property</Link>
        </div>
      </nav>
      ...
      <section className="py-16 bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
            {[
              { label: 'Verified PGs', value: '1,200+' },
              { label: 'Happy Students', value: '5,000+' },
              { label: 'Smart Cities', value: '8' },
              { label: 'Daily Inquiry', value: '300+' }
            ].map((stat) => (
      ...
      <section className="max-w-7xl mx-auto py-32 px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {cities.map((city, i) => (
            ...
            >
              <img src={city.img} className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt={city.name} />
                ...
                >
                  ...
                  <div className="p-12 flex-1 flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap items-center justify-between border-t border-slate-50 pt-10 mt-4 gap-6">
                      ...
                      <Link href={pg.isGoogle ? pg.url : `/pg/${pg.id}`} target="_blank">
                        <button className="bg-slate-900 text-white px-12 py-5 rounded-[2rem] font-black text-sm hover:bg-blue-600 transition-all shadow-xl shadow-blue-100 uppercase tracking-widest">
                          {pg.isGoogle ? 'View on Google Maps' : 'View Full Details'}
                        </button>
                      </Link>
                    </div>
                  </div>
